...
class MajestroNavMeshEnv(gym.Env):
    ...
    def __init__(
        ...
        goal_spawn_min_scale: float = 1.2,
        agent_spawn_min_scale: float = 2.0,
        agent_spawn_max_scale: float = 3.0,
        max_steps: int = 512,
        seed: int = 1,
    ...
    def _compute_assigned_roles(self) -> np.ndarray:
        return np.full((self.num_agents,), ROLE_BASE_MOVE, dtype=np.int32)
    ...
    def _role_reward_bonus(
        ...
        goal_dist = float(np.linalg.norm(self.goal_pos - new_pos))
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        min_goal_dist = max(
            self.step_size * 2.0,
            float(self.sense_radius) * float(self.goal_spawn_min_scale),
        )
        ...
        for idx in candidate_indices:
            ...
            if goal_dist >= min_goal_dist:
                if (start_geo > best_start_geo + 1e-6) or (
                    abs(start_geo - best_start_geo) <= 1e-6 and goal_dist > best_goal_dist
                ):
                    best_goal = goal_rc
                    best_geo = geo_map
                    best_goal_dist = goal_dist
                    best_start_geo = start_geo
            elif best_goal is None and goal_dist > best_goal_dist:
                best_goal = goal_rc
                best_geo = geo_map
                best_goal_dist = goal_dist
                best_start_geo = start_geo
