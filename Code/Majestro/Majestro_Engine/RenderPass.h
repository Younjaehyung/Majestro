...
inline GBUFFER_INDEX ToGBufferIndex(RENDER_TARGET_GROUP_TYPE type, uint32 subRtIndex = 0)
{
	...
	case RENDER_TARGET_GROUP_TYPE::G_BUFFER:   return static_cast<GBUFFER_INDEX>(static_cast<uint8>(GBUFFER_INDEX::GBUFFER_NORMAL_INDEX) + subRtIndex);
...
public:
	...
	virtual void ExecuteIndependentGraphics(const RenderContext& ctx) {}
	virtual void ExecuteDependentGraphics(const RenderContext& ctx) { Execute(ctx); }
