...
//    // [수정] 매크로/디테일/노이즈 타일링 상수(우선 하드코딩)
//    //       - UE 예시처럼 0.1(매크로), 2.0(디테일) 느낌
//    //       - 나중에 PassParams나 MATERIALINFO에 스칼라로 빼는 걸 추천
//    const float kMacroTiling = 0.1f;
//    const float kDetailTiling = 2.0f;
//    const float kNoiseTiling = 1.0f; // 노이즈 반복이 보이면 0.25~2.0 범위에서 조절
...
//        float w = saturate(maskTex.r); // 필요 시 채널 변경

//        if (w <= 1e-4f)
//            continue;

//        // [수정] 노이즈 샘플링 (DiffuseMap2Index)
//        //       - 반복감 억제를 위해 "patch local uv"보다 "fulluv(전체 UV)"를 쓰는 쪽이 일반적으로 더 안정적
//        float noiseAlpha = 0.5f; // 노이즈 없을 때 중간값
//        if (material.DiffuseMap2Index >= 0)
//        {
//            noiseAlpha = TextureMaps[material.DiffuseMap2Index]
//                .Sample(g_sam_0, input.fulluv * kNoiseTiling).r;
//            noiseAlpha = saturate(noiseAlpha);
//        }

//        // [수정] 알베도: 같은 DiffuseMap0를 매크로/디테일 2번 샘플 -> 노이즈로 Lerp
//        float3 layerColor = 0.0f;
//        if (material.DiffuseMap0Index >= 0)
//        {
//            float2 uvMacro = input.uv * kMacroTiling; // [수정]
//            float2 uvDetail = input.uv * kDetailTiling; // [수정]

//            float3 colA = TextureMaps[material.DiffuseMap0Index].Sample(g_sam_0, uvMacro).rgb; // [수정]
//            float3 colB = TextureMaps[material.DiffuseMap0Index].Sample(g_sam_0, uvDetail).rgb; // [수정]
//            layerColor = lerp(colA, colB, noiseAlpha); // [수정]
//        }

//        // [수정] 노멀도 동일하게 매크로/디테일 샘플 후 Lerp (원하는 경우에만 유지)
...
//            float3 tsnA = TextureMaps[material.NormalMapIndex].Sample(g_sam_0, uvMacro).xyz; // [수정]
//            float3 tsnB = TextureMaps[material.NormalMapIndex].Sample(g_sam_0, uvDetail).xyz; // [수정]
//            tsnA = tsnA * 2.0f - 1.0f; // [수정]
//            tsnB = tsnB * 2.0f - 1.0f; // [수정]

//            float3 tsn = normalize(lerp(tsnA, tsnB, noiseAlpha)); // [수정]
//            layerViewNormal = normalize(mul(tsn, matTBN)); // TS -> View
//        }
