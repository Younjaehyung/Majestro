...
void HeightField::Load(const wstring& path)
{
    ...
    if (extension == ".raw")
    {
        throw std::runtime_error("RAW 로드는 크기 정보가 필요합니다: " + filePath.string());
    }

    throw std::runtime_error("지원하지 않는 HeightField 형식: " + filePath.string());
}
...
void HeightField::LoadHeightFieldFromRaw16(const std::string& rawPath, int width, int height, bool littleEndian)
{
    if (width <= 0 || height <= 0)
    ...
    const size_t sampleCount = static_cast<size_t>(width) * static_cast<size_t>(height);
    ...
    if (static_cast<size_t>(file.gcount()) != expectedBytes)
		throw std::runtime_error("RAW 로드 실패: " + rawPath + "에서 예상된 바이트 수를 읽지 못함");
