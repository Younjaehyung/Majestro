...
void Mesh::CreateVertexBuffer(const vector<Vertex>& buffer)
{
	...
	_vertexBuffer->Map(0, &readRange, &vertexDataBuffer);	//메모리 연결
	::memcpy(vertexDataBuffer, &buffer[0], bufferSize);	//gpu로 메모리 복사
	//_vertexBuffer->Unmap(0, nullptr);	//메모리 연결 해제

	// 리소스(데이터) 정보(속성) 설정
	// Initialize the vertex buffer view.
	_vertexBufferView.BufferLocation = _vertexBuffer->GetGPUVirtualAddress();	// GPU VRAM의 주소를 지정
...
void Mesh::Render(uint32 instanceCount, uint32 idx, uint32 baseInstance )
{
	...
	GRAPHICS_CMD_LIST->DrawIndexedInstanced(_vecIndexInfo[idx].count, instanceCount, baseInstance, 0, 0);	//인덱스로 그림을 그려라
}
