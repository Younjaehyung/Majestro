...
void Texture::Load(const wstring& path)
{
	...
	mDescription = CD3DX12_RESOURCE_DESC::Buffer(bufferSize);
...
void Texture::CreateFromResource(ComPtr<ID3D12Resource> tex2D)
{
	...
	if (mDescription.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)
	{
		...
		D3D12_CPU_DESCRIPTOR_HANDLE hDSVHandle = mDsvHeap->GetCPUDescriptorHandleForHeapStart();
		DEVICE->CreateDepthStencilView(mImage.Get(), nullptr, hDSVHandle);
	}
	else
	{
		if (mDescription.Flags & D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET)
		{
			// RTV
			D3D12_DESCRIPTOR_HEAP_DESC heapDesc = {};
			...
			D3D12_CPU_DESCRIPTOR_HANDLE rtvHeapBegin = mRtvHeap->GetCPUDescriptorHandleForHeapStart();
			DEVICE->CreateRenderTargetView(mImage.Get(), nullptr, rtvHeapBegin);
		}
